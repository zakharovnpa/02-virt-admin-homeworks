...
### 6План действий
● Рассмотрим работу с ресурсами облачных провайдеров через
командную строку и веб-консоль
...
● Пользуемся бесплатным тарифом, который доступен год после
регистрации
● Можно зарегистрировать отдельный «учебный» аккаунт на
email типа «yourname+netology@gmail.com»
...
AWS охватывает 77 зон доступности в 24 географических регионах
по всему миру.
...
Это логически изолированный раздел облака, в котором можно
запускать ресурсы в самостоятельно заданной виртуальной сети.
Таким образом можно полностью контролировать среду
виртуальной сети, в том числе выбирать собственный диапазон
IP‑адресов, создавать подсети, а также настраивать таблицы
маршрутизации и сетевые шлюзы.
...
### 17Регистрируем этого пользователя локально
...
Это веб‑сервис, предоставляющий безопасные масштабируемые
...
### 20Создание EC2 через веб интерфейс
https://us-west-2.console.aws.amazon.com/ec2/v2/home

### 21Создание EC2 через консоль
...
● Часто надо привести инстанс в исходное состояние после
ручных правок
...
○ modify.
● Хорошо бы понять, что конкретно будет изменено (типа git
diff)
● Часто надо привести инстанс в исходное состояние после
ручных правок
...
### 26Синтаксис Terraform

### 27Terraform
Terraform – это просто API-клиент.
Terraform-провайдеры «знают» все эти команды и умеют
приводить состояние ресурсов к описанному в своих
...
Плюс много неофициальных, и можно достаточно просто
...
Для того что бы прочитать данные из внешнего API и использовать
